...
function updatePageViews() {
    ...
    const pageViewsRef = db.ref('transazioni/pageViews'); // Modificato il percorso
    ...
    // Modifica la parte che aggiorna l'UI nel metodo updatePageViews
    pageViewsRef.on('value', (snapshot) => {
        const views = snapshot.val();
        const viewCounter = document.getElementById('view-counter');
        if (viewCounter) {
            viewCounter.textContent = `cod.${views || 0}`;
        }
    });
}
